...
const partlyUnlocked: readonly MatchCandidateView[] = first
  ? [
      {
        ...first,
...
// SCR-17 오늘의 인연 Top 3 — 10/T4 퍼블리싱. 잔액·추천·리롤 연결은 10/T2·T3, 해금·운명의 실 모달은 Phase 11.
export const preview: PreviewScreen = {
  ...
  states: {
    ...
    '카드 뒷면(일부 해금)': () => (
      <Cards face="back" view={{ ...base, candidates: partlyUnlocked }} />
    ),
    '카드 뒷면(전체 해금)': () => <Cards face="back" view={{ ...base, candidates: allUnlocked }} />,
    '인연 없음': () => <Cards view={{ ...base, candidates: [] }} />,
    '리롤 무료 o': () => <Cards isRerollOpen view={{ ...base, reroll: { kind: 'free' } }} />,
    '리롤 무료 x': () => <Cards isRerollOpen />,
    '리롤 잔액 부족': () => (
      <Cards
        isRerollOpen
        view={{ ...base, balance: 2, reroll: { kind: 'paid', cost: 3, canAfford: false } }}
      />
    ),
  },
};

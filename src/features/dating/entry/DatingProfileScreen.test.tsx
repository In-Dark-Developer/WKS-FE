...
const { createResultMock, createDatingProfileMock, uploadDatingPhotoMock } = vi.hoisted(() => ({
  createResultMock: vi.fn(),
  createDatingProfileMock: vi.fn(),
  uploadDatingPhotoMock: vi.fn(),
}));
...
test('사주가 있으면 (2/2) 만 받아 그 사주로 저장하고 Top 3 로 간다', async () => {
  ...
  expect(createDatingProfileMock).toHaveBeenCalledWith({ photoId: PHOTO_ID, ...expectedDetails });
});

test('저장이 실패하면 입력값을 둔 채 안내하고, 다시 저장할 때 사주를 새로 만들지 않는다', async () => {
  vi.spyOn(console, 'error').mockImplementation(() => {});
  createDatingProfileMock.mockResolvedValueOnce({ ok: false, error: { kind: 'network' } });
  ...
  await vi.waitFor(() => expect(router.state.location.pathname).toBe('/dating/cards'));
  // 사주는 한 번만 만든다 — 재시도가 새 결과를 만들면 계정에 붙는 사주가 흔들린다.
  expect(createResultMock).toHaveBeenCalledTimes(1);
  expect(createDatingProfileMock).toHaveBeenCalledTimes(2);
});

test('사진 업로드가 실패하면 사진 칸에 알리고 저장하지 않는다', async () => {
  vi.spyOn(console, 'error').mockImplementation(() => {});
  uploadDatingPhotoMock.mockResolvedValue({ ok: false, error: { kind: 'network' } });
  renderScreen({ initialStep: 2, resultId: RESULT_ID, saju: filledSaju });

  fireEvent.change(screen.getByLabelText('사진 추가'), {
    target: { files: [new File(['x'], 'me.jpg', { type: 'image/jpeg' })] },
  });
  await vi.waitFor(() => expect(uploadDatingPhotoMock).toHaveBeenCalled());
  fireEvent.click(screen.getByRole('button', { name: '내 운명 찾아 떠나기' }));

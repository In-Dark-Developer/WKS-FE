...
import { mbtiOptions } from './options';
...
export function PreRegisterForm({ defaultValues, onDone }: Props) {
  ...
  const connectionFailed = actionData.success && !submitting;
  ...
  return (
    <form className="flex flex-col gap-24 pt-24" noValidate onSubmit={handleSubmit}>
      ...
      <fieldset className="flex flex-col gap-24 disabled:opacity-80" disabled={submitting}>
        ...
        <Field
          error={errors.phone}
          help="연락받을 전화번호를 숫자만 입력해 주세요."
          label={<ErrorLabel invalid={Boolean(errors.phone)}>전화번호</ErrorLabel>}
        >
          {(control) => (
            <TextField
              {...control}
        ...
        <Field
          ...
          help="매칭이 성립한 상대에게만 보여요. 없으면 비워 두세요."
          label={
            <ErrorLabel invalid={Boolean(errors.instagram)}>인스타그램 아이디 (선택)</ErrorLabel>
          }
        >
          {(control) => (
            <TextField
      ...
      <Button
        ...
        variant="accent"
      >
        {connectionFailed ? '다시 신청하기' : '사전 신청하기'}
      </Button>
    </form>
  );
}

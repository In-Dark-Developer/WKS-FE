...
1. 사주 보기 — 입력 폼(`features/saju`) → `api`가 요청 스키마로 검증해 POST → 응답을 zod로 파싱 → 결과 화면이 운명 카드(운명 제목·설명, 연애운·결혼운·자녀운 등급, 행운의 장소·아이템)를 렌더 → 공유 링크 id와 인연카드 등급(`cardGrades`)를 `features/share`에 넘긴다.
2. 친구 궁합 — 공유 링크(`/s/:shareId`)로 진입 → 링크 주인의 닉네임·십이간지를 조회(사주 요약은 없다) → 방문자가 자기 사주를 입력 → 궁합 점수 응답(점수 + 등급)을 받아 표시하고, 양쪽의 궁합 지도(`features/friends`: 구슬·등급별 인원·순위)에 반영된다. 등급별 인원 수는 `/me/friends` 목록에서 `lib`의 순수 함수가 센다.

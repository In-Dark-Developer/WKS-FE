...
| 정적 호스팅      | SPA 배포와 공유 링크 라우팅       | Cloudflare Workers 정적 자산(`wrangler.jsonc`) — `https://threatoffate.site`, 모르는 경로는 `index.html` (ADR-20260913-workers-static-assets) | 배포 실패 시 직전 버전 유지. 공유 링크 미리보기(NFR-3)는 Phase 08 |

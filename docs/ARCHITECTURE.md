...
| 정적 호스팅      | SPA 배포와 공유 링크 라우팅       | AWS S3 + CloudFront, DNS Route53 — `https://threatoffate.site`, 403·404 는 `index.html` 200. `main` 병합마다 GitHub Actions 배포 (ADR-20260914-aws-cloudfront-hosting, `docs/deploy/aws-frontend.md`) | 배포 실패 시 이전 파일 그대로(업로드 전 실패는 영향 없음). 공유 링크 미리보기(NFR-3)는 Phase 08 |

...
1. 사주 보기 — 입력 폼(`features/saju`) → `api`가 요청 스키마로 검증해 POST → 응답을 zod로 파싱 → 결과 화면이 운명 카드(운명 제목·설명, 연애운·결혼운·자녀운 등급, 행운의 장소·아이템)를 렌더 → 공유 링크 id와 인연카드 데이터(`cardScores`)를 `features/share`에 넘긴다.
2. 친구 궁합 — 공유 링크(`/s/:shareId`)로 진입 → 링크 주인의 요약을 조회 → 방문자가 자기 사주를 입력 → 궁합 점수 응답(점수 + 등급)을 받아 표시하고, 양쪽의 궁합 지도(`features/friends`: 구슬·등급별 인원·순위)에 반영된다. 등급별 인원 수는 `/me/friends` 목록에서 `lib`의 순수 함수가 센다.
3. 운명의 실 — 결과 화면의 사전신청 티저 → 모달(`features/profile`)에서 추가 정보 등록·동의(동의 전에는 전송하지 않는다) → 후보 카드 열람(`features/matching`) → '보내기' 호출 → 상대가 '당기기'를 하면 성립 응답에 연락처가 포함되어 화면에 공개된다.
...
브라우저에 저장하는 것은 세션 식별자뿐이다(인트로는 MVP에서 제외됐다). 사용자 데이터·사주 결과·매칭 상태는 모두 백엔드가 저장하며 이 저장소에는 스키마·마이그레이션이 없다.

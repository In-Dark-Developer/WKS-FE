...
- **AC5 문자 그대로는 미충족** — `src/app/routes.tsx` 를 T3·T7·T8 외에 네 커밋이 고쳤다. 충돌·되돌림은 없었다.
  ...
  - 737b218 04/T7(2026-09-15): 인연카드를 결과 화면에 합치며 `/reading/:id/card`·`cardLoader` 제거
...
| AC1 | 운영 `https://threadoffate.site`(main 32fda55 배포본)에서 playwright(390px)로 입력 → 결과 → 인연카드 → 결과 복귀 1회 (2026-09-15) | pass — `POST /api/results 201`, `GET /api/results/{id} 200`, 콘솔 에러 0 (운영 DB 1건·LLM 1회 생성, 닉네임 '점검'). 그 뒤 04/T7(#94)이 인연카드를 결과 화면에 합쳤다 — 입력 → 결과 경로는 `routes.test` 로 재확인 |
...
| AC6 | `routes.test` 보관값 없음·다른 `resultId`·같은 `resultId` + 운영에서 새 브라우저 컨텍스트로 결과 주소 열기 | pass — 새 컨텍스트는 `/` 로 이동. `/reading/:id/card` 는 04/T7 이 없앴다(없는 경로 화면) |
...
- 운명 제목·설명 말줄임(운영 응답 '삼재를 꿰뚫는 붉…')은 04/T7 이 전문 표시(합 120자)로 고쳤다.

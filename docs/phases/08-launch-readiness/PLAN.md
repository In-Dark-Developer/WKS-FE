...
- Cloudflare Pages 배포 설정 — 빌드 설정·SPA 기본 동작 유지(최상위 `404.html` 없음)·자산 캐시(`public/_headers`)·운영 API 주소
- 운영 연결 — Pages 프로젝트 연결, `threatoffate.site` 네임서버 이전·DNS·HTTPS, 백엔드 CORS 확인 (사람 작업)
...
- ADR-20260913-cloudflare-pages-hosting — 프론트 Cloudflare Pages `threatoffate.site`, 백엔드 `api.threatoffate.site` (hosting-and-domains 대체, 병합됨 PR #67)
- 현재 빌드(2026-09-13): 초기 JS 130KB gzip(예산 250KB 안), CSS 23KB gzip, 폰트 동국체 434KB·성곡체 205KB(첫 화면 LCP 에 영향 — T5 에서 측정)

**사람·다른 팀이 해야 하는 것**

- Cloudflare 계정·Pages 프로젝트 — Workers & Pages → Create → Pages → Connect to Git 으로 `In-Dark-Developer/WKS-FE` 연결(조직에 Cloudflare Pages GitHub 앱 설치 — 조직 관리자 권한). 빌드 명령 `pnpm build`, 출력 `dist`, 환경변수 `VITE_API_BASE_URL=https://api.threatoffate.site/api`·`PNPM_VERSION=11.22.0`(Production·Preview 둘 다). 연결 뒤에는 `main` 병합마다 자동 배포, PR 마다 미리보기
- DNS — `threatoffate.site` 네임서버를 Cloudflare 로 옮기기 전에 기존 레코드(`api`, 메일을 쓰면 MX·TXT)를 Cloudflare DNS 에 만든다. apex·`www` 는 Pages 프로젝트 Custom domains 에서 추가, `api` 는 백엔드 서버로 DNS only
...
- [x] T2. Cloudflare Pages 배포 설정 — Done when: 해시 자산 장기 캐시 `public/_headers` 가 병합되고, 산출물에 최상위 `404.html` 이 없어 Pages 기본 SPA 라우팅이 동작하며, 대시보드 빌드 설정(빌드 명령·출력·`VITE_API_BASE_URL`·`PNPM_VERSION`)이 Dependencies 에 적혀 사이트 연결 때 그대로 넣으면 된다 · Touches: `public/_headers` · Owner: @jjjung0921 (commit 1a48ffc — 소유자 요청으로 T1 계획 PR 에 함께)

- [ ] T3. 운영 연결 확인 — Done when: Cloudflare Pages 프로젝트가 `WKS-FE` 에 연결되고 첫 배포 로그에서 Node(.nvmrc)·pnpm 11 빌드가 성공하며, `https://threatoffate.site` 가 HTTPS 로 열리며, `https://threatoffate.site/s/test` 새로고침이 404 가 아니고, 운영 화면에서 `api.threatoffate.site` 호출이 CORS 오류 없이 가며, `www.` 가 apex 로 넘어간다 — 확인 결과를 RESULT 에 적는다 · Touches: `docs/phases/08-launch-readiness/` · After: T2 · Owner: @jjjung0921

- [ ] T4. 공유 링크 미리보기 — Done when: `index.html` 에 서비스 공통 OG·Twitter 메타(제목·설명·썸네일·`og:url`)와 썸네일 이미지가 있어 카카오톡·인스타·메신저에 `https://threatoffate.site/s/<id>` 를 붙이면 제목·설명·썸네일이 보이고(SC-6), `shareId` 별 동적 미리보기(Pages Functions vs 백엔드 렌더)는 ADR 로 채택·기각이 남는다 · Touches: `index.html`, `public/og/`, `docs/decisions/` · After: T3 · Owner: @jjjung0921
...
<!-- T2 는 소유자 지시(2026-09-13 "Phase 08 계획이랑 같이 올려줘")로 계획 PR 에 함께 들어갔다. 호스팅을 Netlify 에서 Cloudflare Pages 로 바꿔(ADR-20260913-cloudflare-pages-hosting) netlify.toml 대신 public/_headers 다. T3·T6 은 사람이 실기기·계정으로 확인하는 Task 라 코드 Touches 가 없다.
...
- `docs/decisions/ADR-20260913-cloudflare-pages-hosting.md` (대체된 `ADR-20260913-hosting-and-domains.md`)
- `docs/ARCHITECTURE.md` — External Systems(정적 호스팅·백엔드), Cross-cutting Concerns(설정)
- `docs/api/openapi.yaml` — servers(운영)

## Acceptance Criteria

- [ ] AC1. `main` 병합이 사람 손 없이 Cloudflare Pages 로 `https://threatoffate.site` 에 배포되고, 공유 링크 직접 진입·새로고침이 앱을 연다
...
- AC1: 이 PLAN 병합 뒤 Cloudflare Pages 배포 목록에서 커밋 SHA 확인, `/s/test` 새로고침 · T3

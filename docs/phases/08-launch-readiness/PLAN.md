...
- Cloudflare Workers 정적 자산 배포 설정 — `wrangler.jsonc`(SPA)·pnpm 빌드 허용·자산 캐시(`public/_headers`)·운영 API 주소
- 운영 연결 — Workers 프로젝트 연결, `threatoffate.site` 네임서버 이전·DNS·HTTPS, 백엔드 CORS 확인 (사람 작업)
...
- ADR-20260913-workers-static-assets — 프론트 Cloudflare Workers 정적 자산 `threatoffate.site`, 백엔드 `api.threatoffate.site` (cloudflare-pages-hosting 대체)
- 현재 빌드(2026-09-13): 초기 JS 130KB gzip(예산 250KB 안), CSS 23KB gzip, 폰트 동국체 434KB·성곡체 205KB(첫 화면 LCP 에 영향 — T5 에서 측정)

**사람·다른 팀이 해야 하는 것**

- Cloudflare 계정·Workers 프로젝트 — 저장소 `In-Dark-Developer/WKS-FE` 연결됨(2026-09-13). Worker 이름은 `wks-fe`(`wrangler.jsonc` 와 같아야 한다). 빌드 명령 `pnpm build`, 배포 명령 `npx wrangler deploy`, 빌드 변수 `VITE_API_BASE_URL=https://api.threatoffate.site/api`·`PNPM_VERSION=11.22.0`. 연결 뒤에는 `main` 병합마다 자동 배포, PR 마다 미리보기 버전
- DNS — `threatoffate.site` 네임서버를 Cloudflare 로 옮기기 전에 기존 레코드(`api`, 메일을 쓰면 MX·TXT)를 Cloudflare DNS 에 만든다. apex·`www` 는 Worker 의 Settings → Domains & Routes → Custom Domain 으로 추가, `api` 는 백엔드 서버로 DNS only
...
- [x] T2. Cloudflare 배포 설정 — Done when: `wrangler.jsonc`(dist 정적 자산·SPA not_found_handling)·`wrangler` 개발 의존성·`pnpm-workspace.yaml` allowBuilds(esbuild·workerd)·해시 자산 캐시 `public/_headers` 가 병합되어 Workers Builds 가 `pnpm build` → `npx wrangler deploy` 로 오류 없이 배포하고, `wrangler deploy --dry-run` 이 로컬에서 dist 를 읽는다 · Touches: `wrangler.jsonc`, `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `.gitignore`, `public/_headers` · Owner: @jjjung0921 (commit 25deaa1 — 첫 배포 ERR_PNPM_IGNORED_BUILDS 로 재작업, ADR-20260913-workers-static-assets)

- [ ] T3. 운영 연결 확인 — Done when: Cloudflare Workers 프로젝트가 `WKS-FE` 에 연결되고 첫 배포 로그에서 Node(.nvmrc)·pnpm 11 빌드가 성공하며, `https://threatoffate.site` 가 HTTPS 로 열리며, `https://threatoffate.site/s/test` 새로고침이 404 가 아니고, 운영 화면에서 `api.threatoffate.site` 호출이 CORS 오류 없이 가며, `www.` 가 apex 로 넘어간다 — 확인 결과를 RESULT 에 적는다 · Touches: `docs/phases/08-launch-readiness/` · After: T2 · Owner: @jjjung0921

- [ ] T4. 공유 링크 미리보기 — Done when: `index.html` 에 서비스 공통 OG·Twitter 메타(제목·설명·썸네일·`og:url`)와 썸네일 이미지가 있어 카카오톡·인스타·메신저에 `https://threatoffate.site/s/<id>` 를 붙이면 제목·설명·썸네일이 보이고(SC-6), `shareId` 별 동적 미리보기(Worker 스크립트 vs 백엔드 렌더)는 ADR 로 채택·기각이 남는다 · Touches: `index.html`, `public/og/`, `docs/decisions/` · After: T3 · Owner: @jjjung0921
...
<!-- T2 는 소유자 지시(2026-09-13 "Phase 08 계획이랑 같이 올려줘")로 계획 PR 에 함께 들어갔다. 호스팅을 Netlify → Cloudflare Pages → Workers 정적 자산(ADR-20260913-workers-static-assets)으로 바꿔 wrangler.jsonc·public/_headers 다. T3·T6 은 사람이 실기기·계정으로 확인하는 Task 라 코드 Touches 가 없다.
...
- `docs/decisions/ADR-20260913-workers-static-assets.md` (도메인 구성은 `ADR-20260913-cloudflare-pages-hosting.md`)
- `docs/ARCHITECTURE.md` — External Systems(정적 호스팅·백엔드), Cross-cutting Concerns(설정)
- `docs/api/openapi.yaml` — servers(운영)

## Acceptance Criteria

- [ ] AC1. `main` 병합이 사람 손 없이 Cloudflare Workers 로 `https://threatoffate.site` 에 배포되고, 공유 링크 직접 진입·새로고침이 앱을 연다
...
- AC1: 이 PLAN 병합 뒤 Cloudflare Workers 배포 목록에서 커밋 SHA 확인, `/s/test` 새로고침 · T3

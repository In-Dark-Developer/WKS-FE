...
- Phase 색인: `Phase`("NN name") · `번호` · `Lead` · `Depends on` · `Status` · `Tasks`(완료/전체) · `Result` · `GitHub`(PLAN.md 링크) · `Goal`(PLAN ## Goal 첫 문단) · `Scope`(PLAN ## Scope 항목을 `·`로) · `Task 보드`(관계 — Task 보드에서 `Phase` select 가 같은 행들, 양방향이라 보드 행에도 `Phase 색인`이 생긴다) — `docs/phases/README.md` 표와 각 PLAN.md 가 원본이고 표는 `ai-stream.sh phases` 가 PLAN 머리에서 만든다. 같은 실행이 PLAN Task 줄의 `After: Tk` 를 Task 보드의 `선행 Task` 관계(양방향 `후행 Task`)로 복사한다.

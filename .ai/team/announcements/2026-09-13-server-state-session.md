...
- Action: 요청/캐시 라이브러리(TanStack Query 등)를 추가하지 않는다. `src/api/` 함수는 라우터를 모르고, 화면은 feature가 export 한 route `loader`(읽기)·`action`/`useFetcher`(쓰기)로 부른다 — 컴포넌트 `useEffect` fetch 금지. `/reading/:id` 하위 화면은 `useRouteLoaderData`로 부모 결과를 쓴다. 세션은 백엔드가 보낸 토큰 하나다 — 로그인은 없고 내 데이터는 토큰으로만 찾으며 `resultId`는 공유 링크에만 쓴다. 토큰은 `src/api/session.ts`의 `readSession`·`writeSession`·`clearSession`만 쓰고 localStorage를 직접 만지지 않는다. POST는 자동 재시도하지 않는다. T1은 `session.ts`·토큰 헤더·GET 재시도·토큰을 주는 목 응답을, T3은 loader 가드·`errorElement`·`HydrateFallback`을 맡는다.
- Until: Phase 08 종료

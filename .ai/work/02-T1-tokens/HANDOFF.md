...
- 토큰 밖 클래스 린트(`better-tailwindcss/no-unknown-classes`) · `docs/CONVENTIONS.md` 4장 클래스 이름 표 · PLAN T1 Owner → @jjjung0921
...
- `eslint.config.js` · `.prettierrc` · `docs/CONVENTIONS.md` · `package.json` · `pnpm-lock.yaml` · `.ai/team/{announcements/2026-09-13-design-tokens.md,README.md}` · ADR · PLAN

## Decisions Made

- 소유자 지시(2026-09-13): T1 을 @jjjung0921 명의로 진행(PLAN Owner 도 @jjjung0921 로 변경), Pretendard 번들, 클래스 이름은 Figma 이름 그대로(`p-16`=16px), Display 폰트 파일은 소유자 제공
...
- Touches 추가: `src/lib/cn.test.ts`(Rule 8), 공지·색인(Rule 14), `docs/CONVENTIONS.md`(소유자 지시)
- eslint-plugin-better-tailwindcss 4.7.0 도입(소유자 승인) — entryPoint `src/index.css`

## Tests Executed

- `pnpm test` · `pnpm typecheck` · `pnpm lint` · `pnpm build` · 임시 파일로 린트 확인(hex·rgb()·[color:] 4건, bg-red-500·p-2·text-sm·rounded-lg·bg-white 5건 실패 / CONVENTIONS 표의 클래스 전부 통과) · dev 서버에서 `document.fonts` 로드 확인
...
- 없음 — 동국체 웹 사용 라이선스는 소유자가 2026-09-13 확인. 원본 OTF 는 CFF 이름 공백 때문에 브라우저 OTS 가 거부해 이름만 `DONGGUKUNIVERSITY` 로 바꿔 woff2 변환

## Exact Next Action

PR 리뷰 코멘트 대응. 병합 후 T2~T5 에 `git merge main` 안내

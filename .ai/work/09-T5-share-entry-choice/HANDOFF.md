...
- Figma v1.0 30:6128 반영: 초대 제목·부제, 주인 궁합 지도(`CompatibilityMap`), 안내 + Button/Primary M + 밑줄 '새로 작성하기'. 새로 작성은 선택 아래 폼(30:6323)
...
- `src/features/friends/shareInputLoader.ts` (+test) — `canReusePrevious`·`ownerFriends`, 자동 join 제거 · `shareMapLoader.ts:toOwnerFriends` 공유
...
- 새로 작성 시 폼 머리글이 기존 문구다 — 디자인(30:6323)은 '사주를 입력해 인연을 확인하세요'·제출 '내 운명을 친구 궁합 지도에 꿰기'. `SajuForm`·`ShareInputScreen` 은 Touches 밖 → FR-15 V1(4.1) 작업과 함께
- 화면 눈 대조 전(테스트·타입·린트만) — deploy preview 에서 확인
...
`git merge dev` → `scripts/ai-end.sh --ready` → deploy preview 에서 30:6128 과 대조
